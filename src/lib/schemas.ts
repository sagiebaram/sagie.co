...
// Matches mockups/contribution-form.html — single privacy consent is a
// submission precondition (z.literal(true)) and is NOT persisted to Notion.

export const CONTRIBUTION_TYPE_VALUES = [
  'mentorship',
  'collaboration',
  'events',
  'fractional',
...
export const ContributeSchema = z.object({
  ...
  email: emailSchema,
  contributionTypes: z
    .array(z.enum(CONTRIBUTION_TYPE_VALUES))
    .min(1, 'Pick at least one area.'),
  description: z.string().max(2000).trim().optional(),
  availability: z.enum(AVAILABILITY_VALUES, {
    error: 'Please select your availability.',
  }),
  // Privacy consent — must be true to submit. Using boolean().refine() rather
  // than z.literal(true) so the form's default value of `false` is a valid
  // input shape that RHF can bind to.
  consent: z.boolean().refine((v) => v === true, {
    message: "You'll need to agree to continue.",
  }),
});
