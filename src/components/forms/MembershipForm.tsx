  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <Controller
        ...
        defaultValue={[]}
        render={({ field }) => (
          <FormField label="What are you interested in? (select all that apply)" name="category" type="checkbox-group" options={['Founder', 'Investor', 'Tech Pro', 'Ecosystem Builder', 'Sponsor', 'Partner', 'Advisor']} value={field.value ?? []} onArrayChange={(v) => field.onChange(v)} error={errors.category?.message} />
        )}
      />
