...
export function ContributeV2Categories() {
  ...
  return (
    <Section id="v2-categories">
      ...
      <h2 className="font-display uppercase text-foreground-secondary text-chapter leading-none tracking-heading mb-4">
        {title}
      </h2>
      <p className="font-body text-foreground-muted text-body font-light leading-body max-w-[560px] mb-16">
        {desc}
      </p>
...
function V2CategoryRow({ cat, isLast }: { cat: ContributeV2Category; isLast: boolean }) {
  return (
    ...
    >
      ...
      <h3 className="font-display uppercase text-foreground-secondary text-chapter leading-none tracking-heading col-start-2 md:col-start-2">
        {cat.name}
      </h3>

      {/* Body — flows to full width on desktop, below name on mobile */}
      <p className="font-body text-foreground-muted text-body font-light leading-body col-start-2 md:col-start-3 md:row-start-1 max-w-[560px]">
        {cat.body}
      </p>
    </li>
  )
